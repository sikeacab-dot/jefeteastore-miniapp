...
const ZenState = {
    products: products, // Default fallback
    cart: {},
    current: { productId: null, variant: null, qty: 1, slide: 0 }
};

// IndexedDB Helper for Index
async function getStoredProducts() {
    return new Promise((resolve) => {
        const request = indexedDB.open('JefeTeaDB', 1);
        request.onsuccess = (e) => {
            const db = e.target.result;
            if (!db.objectStoreNames.contains('products')) return resolve(null);
...
const ZenUI = {
    async init() {
        // Try to load latest from IndexedDB first
        const stored = await getStoredProducts();
        if (stored) {
            ZenState.products = stored;
        } else {
            // Legacy/Fallback check
            const s = localStorage.getItem('jefe_products');
            if (s) { try { ZenState.products = JSON.parse(s); } catch (e) { } }
        }
    ...
    openCart() {
        ...
        if (items) { items.style.removeProperty('display'); items.classList.remove('hidden'); }
        if (footer) { footer.style.removeProperty('display'); footer.classList.remove('hidden'); }
    ...
    async processCheckout() {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const checkoutFooterEl = document.getElementById('checkout-footer');
                if (cartItemsEl) { cartItemsEl.style.removeProperty('display'); cartItemsEl.classList.add('hidden'); }
                if (checkoutFooterEl) { checkoutFooterEl.style.removeProperty('display'); checkoutFooterEl.classList.add('hidden'); }
